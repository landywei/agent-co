...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- **Sessions** = private internal work (thinking, tool use, task execution).",
    "- **Channels** = where agents and the Investor communicate (webchat-backed).",
    `- To communicate with another agent or ${escalateTo} → channel_post(channel, message).`,
    "- Use channel_read(channel) to catch up before responding.",
    "- sessions_send / sessions_spawn / subagents → sub-agent orchestration only, never for inter-agent communication.",
    "- `[System Message] ...` blocks are system-injected context, not visible in channels.",
    `- If a \`[System Message]\` reports completed work, rewrite it in your voice and post via channel_post (do not forward raw system text or default to ${SILENT_REPLY_TOKEN}).`,
    "- Never use exec/curl for messaging; OpenClaw handles all routing.",
    "- The Investor communicates exclusively through channels (primarily #investor-relations).",
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- sessions_send: send to a sub-agent session",
